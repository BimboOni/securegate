...
function PasswordInput({
  ...
  return (
    <div className="relative rounded-xl shadow-sm">
      <input type={showPassword ? "text" : "password"} name="password" placeholder="Password" required className={`${className} pr-14`} value={value} onChange={onChange} onBlur={onBlur} />
      <div className="absolute inset-y-0 right-0 pr-4 flex items-center">
        <button type="button" onClick={onToggle} className="text-zinc-500 hover:text-zinc-300 focus:outline-none cursor-pointer p-1">
          {showPassword ? (
...
function ForgotPasswordForm({ shared, onBack }: { shared: FormShared; onBack: () => void }) {
  ...
  return (
    <form className="space-y-5" onSubmit={(e) => e.preventDefault()} noValidate>
      ...
      <button type="submit" className="w-full flex justify-center py-3.5 px-6 border border-transparent rounded-xl shadow-lg shadow-blue-500/5 text-sm bg-zinc-100 text-zinc-950 font-semibold hover:bg-blue-600 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 cursor-pointer active:scale-[0.98]">
        Send reset link
      </button>
      <div className="text-center text-sm pt-1">
        <button type="button" onClick={onBack} className="font-medium text-blue-400 hover:text-blue-300 transition-colors cursor-pointer">Back to sign in</button>
      </div>
    </form>
  );
}
...
function AuthContent() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    e.preventDefault(); setError("");
    setSubmitted(true);
    if (!formData.email.trim()) { setFieldErrors((prev) => ({ ...prev, email: "Field cannot be empty" })); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) { setFieldErrors((prev) => ({ ...prev, email: "Enter A Valid Email Address" })); return; }
    ...
    <div className="w-full max-w-[clamp(16rem,38vw,24rem)]">
      ...
      {view === "forgot" && forgotStatus === "success" ? (
        <div className="text-center py-6">
          ...
          <button type="button" onClick={() => switchView("login")} className="text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors cursor-pointer">Return to sign in</button>
        </div>
      ) : (
        <div>
          {formError && (
...
export default function AuthPage() {
  return (
    <Suspense fallback={<div className="w-full max-w-[clamp(16rem,38vw,24rem)] text-center"><p className="text-sm text-zinc-500">Loading...</p></div>}>
      <AuthContent />
    </Suspense>
  );
}
